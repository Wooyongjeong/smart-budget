begin;
select plan(4);
select ok(has_table('public','voucher_movements'),'voucher movements table exists');
select ok(has_function('public','record_voucher_event',array['uuid','text','uuid','bigint','date','text','uuid','uuid']),'voucher event RPC exists');
