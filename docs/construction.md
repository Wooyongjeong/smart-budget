...
설정의 공동 가계부 화면에서 현재 구성원을 확인하고, 이름이 비어 있으면 본인은 `나`, 상대는 `배우자`로 표시한다. 본인 이름은 `update_profile_display_name` RPC로 수정할 수 있다. 7일 유효 초대 코드를 생성·복사하거나 받은 48자리 코드를 수락할 수 있으며, 서버 오류를 만료/재사용·이미 참여·정원 초과·접근 불가 안내로 구분한다. 탈퇴는 영향 확인 뒤 RPC를 실행하며 성공 시 공동 데이터 참조를 비우고 Supabase 세션을 종료한다. 초대 코드는 앱에 영구 저장하지 않는다.
