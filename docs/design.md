...
초대 링크는 HTTPS Universal Link/App Link로 앱을 열고, 개발용 `smartbudget://invite/<token>`도 지원한다. 비로그인 사용자의 토큰은 로그인 완료 전까지 메모리에만 보관한다. 신규 사용자는 카카오 로그인 성공 후 카카오 닉네임 또는 기본값 `나`를 확인·수정하고, 다음 코드 입력 화면에서 보관한 초대 코드를 자동으로 채운다. 사용자는 참여 또는 건너뛰기를 선택하며, 서버 수락은 기존 RPC에서 가계부 잠금·2명 제한·만료·일회성을 함께 검사한다. 구성원 테이블에 직접 insert/update하거나 household_id를 바꾸어 참여할 수 없어야 한다. 초대 무차별 시도 제한도 둔다.
