...
서버 오류는 `invitation_invalid`, `invitation_expired_or_used`, `already_member`, `household_full`, `household_unavailable`, `forbidden`을 사용자 행동에 맞는 안내로 변환한다. 수락 성공 시 새 가계부 문맥을 다시 읽고, 탈퇴 성공 시 화면의 공동 데이터를 즉시 비운 뒤 인증 화면으로 돌아간다. 앱은 구성원 테이블을 직접 변경하지 않고 기존 `create_invitation`, `accept_invitation`, `leave_household` RPC만 호출한다.
