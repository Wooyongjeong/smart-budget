...
- 카카오 로그인·로그아웃, 공동 가계부 DB, 일반 거래 저장, 월 조회, 결제 수단, 카드 목표와 상품권 잔액이 연결되어 있다.
- 이용내역 캡처 화면은 고정 fixture이며 실제 이미지 선택·AI 공급자 연결은 미완성이다.
- 초대 코드 생성·수락, 구성원 확인, 탈퇴 앱 UI가 연결되어 있다. 초대 링크 수신과 로그인 후 자동 입력도 연결되어 있다. 다른 기기 변경의 실시간 동기화는 없다.
...
이번 작업은 설정의 `계정 · 공동 가계부`에서 현재 구성원을 확인하고, 7일 유효 초대 코드를 생성·복사하거나 받은 코드를 수락하고, 영향 안내를 확인한 뒤 본인이 가계부를 나가는 흐름까지 연결한다. 입력은 로그인 세션, 현재 가계부 ID, 48자리 초대 코드이며 출력은 갱신된 구성원 목록 또는 가계부 접근 종료 상태다. 초대 원문은 생성 응답에서만 표시하고 앱에 영구 저장하지 않는다.

서버 오류는 `invitation_invalid`, `invitation_expired_or_used`, `already_member`, `household_full`, `household_unavailable`, `forbidden`을 사용자 행동에 맞는 안내로 변환한다. 이름이 없는 본인 구성원은 `나`, 상대 구성원은 `배우자`로 표시하고, 본인 표시 이름은 `update_profile_display_name` RPC로 수정한다. 첫 가계부 생성은 이미 저장한 이름을 빈 값으로 덮어쓰지 않으며, 상단 프로필 원형과 구성원 목록은 같은 표시 이름을 사용한다. 수락 성공 시 새 가계부 문맥을 다시 읽고, 탈퇴 성공 시 화면의 공동 데이터를 즉시 비운 뒤 인증 화면으로 돌아간다. 앱은 구성원·프로필 테이블을 직접 변경하지 않고 검증된 RPC만 호출한다.

이번 작업의 완료 기준은 (1) 한 명일 때 초대 생성과 코드 복사, (2) 유효 코드 수락 후 두 구성원 표시, (3) 잘못됨·만료/재사용·이미 참여·2명 정원 오류 안내, (4) 확인 없는 탈퇴 방지와 성공 후 로컬 데이터 제거·로그아웃, (5) repository 및 위젯 회귀 테스트와 전체 Flutter 정적 분석/테스트 통과다. 다른 기기의 변경을 자동 반영하는 Realtime 구독은 후속 T09 항목으로 남긴다.

## 초대 링크·가입 연결 계약

초대 화면의 공유 대상은 `https://<초대 도메인>/invite/<48자리 토큰>`이며 OS 공유 시트에서 카카오톡을 선택할 수 있다. 앱은 HTTPS Universal Link/App Link와 개발용 `smartbudget://invite/<토큰>`을 모두 수신하고, 로그인 전에는 토큰을 메모리에 보관한다. 카카오 로그인 성공 뒤 신규 사용자는 먼저 가계부에서 사용할 이름을 확인·수정한다. 카카오 프로필 닉네임 이용 동의가 있으면 닉네임을 초깃값으로 사용하고, 없으면 `나`를 사용한다. 이름 저장 뒤 초대 코드 입력 화면에서 코드를 자동 입력한 상태로 참여 또는 건너뛰기를 선택한다. 토큰 원문은 영구 저장하지 않는다. 플랫폼 파일과 배포 설정은 [deep-links.md](deep-links.md)에 기록한다.

완료 기준은 (1) 공유 버튼이 전체 링크를 전달하고, (2) 콜드 스타트/실행 중 링크에서 48자리 토큰을 추출하며, (3) 비로그인 링크 클릭 후 카카오 로그인 뒤 코드가 자동 입력되고, (4) 참여 성공 후 기존 공동 가계부 화면으로 진입하며, (5) 코드 없는 일반 로그인은 기존 앱 진입을 유지하는 것이다. 실제 HTTPS 도메인, iOS Associated Domains, Android assetlinks, 카카오톡 공유 템플릿/개발자 콘솔 설정은 배포 환경에서 별도 검증한다.
