...
현재 코드의 문서 대비 수정·미완료 항목은 [code-review.md](code-review.md)를 먼저 확인한다. 아래 PR #1/#2 및 T01 시작 안내는 초기 인계 시점의 기록이며 현재 완료 상태가 아니다. T08/T09/T12는 코드 존재나 기본 테스트 통과만으로 완료 처리하지 않는다.
...
| T13 | 출시 점검 | 모두 | 실기기, 계정 삭제/보존, AI 비용/품질, 운영 오류 확인 |
