...
- 초대 코드 생성·수락, 구성원 확인, 탈퇴 앱 UI가 연결되어 있다. 다른 기기 변경의 실시간 동기화와 로그인 전 초대 딥링크는 없다.
...
이번 작업의 완료 기준은 (1) 한 명일 때 초대 생성과 코드 복사, (2) 유효 코드 수락 후 두 구성원 표시, (3) 잘못됨·만료/재사용·이미 참여·2명 정원 오류 안내, (4) 확인 없는 탈퇴 방지와 성공 후 로컬 데이터 제거·로그아웃, (5) repository 및 위젯 회귀 테스트와 전체 Flutter 정적 분석/테스트 통과다. 다른 기기의 변경을 자동 반영하는 Realtime 구독과 로그인 전 딥링크로 초대 의도를 보존하는 흐름은 후속 T09 항목으로 남긴다.
