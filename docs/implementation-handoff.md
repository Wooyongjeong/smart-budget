...
서버 오류는 `invitation_invalid`, `invitation_expired_or_used`, `already_member`, `household_full`, `household_unavailable`, `forbidden`을 사용자 행동에 맞는 안내로 변환한다. 이름이 없는 본인 구성원은 `나`, 상대 구성원은 `배우자`로 표시하고, 본인 표시 이름은 `update_profile_display_name` RPC로 수정한다. 수락 성공 시 새 가계부 문맥을 다시 읽고, 탈퇴 성공 시 화면의 공동 데이터를 즉시 비운 뒤 인증 화면으로 돌아간다. 앱은 구성원·프로필 테이블을 직접 변경하지 않고 검증된 RPC만 호출한다.
